...
*   `storage` **Storage\<K, V>** cache storage that implements CacheStorage\<K, V> interface \[must provide get(key) and set(key, value, expired) methods] (optional, default `InMemoryStorageWrapper`)
