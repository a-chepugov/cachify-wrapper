 ...
 * @param {Number} [options.expire.deviation=options.expire.ttl/100] - `ttl` deviation (prevents simultaneous keys deletions) [in milliseconds]
 * @param {Number} [options.lock=1000] - lock timeout (prevents simultaneous concurrent invoke of `fn` at initial period) [in milliseconds]
 * @param {Boolean|Object} [options.stale] - allow to use a stale data
 * @param {Number} [options.stale.lock=options.lock] - lock timeout for updating stale data [in milliseconds]
