...
module.exports = function (fn, cache, {expire: {ttl, deviation} = {}, expire, lock, stale: {lock: staleLock} = {}, stale, hasher, timeout, latency, retries} = {}, thisArg) {
	...
	lock = Number.isFinite(lock) && lock > 0 ? lock : 1000;

	latency = Number.isFinite(latency) && latency > 0 ? latency : lock;
	retries = Number.isFinite(retries) && retries > 0 ? retries : Math.floor(lock / latency) + 1;

	staleLock = staleLock && Number.isFinite(staleLock) && staleLock > 0 ? staleLock : lock;
	...
	const cacheLock = (key) => cacheSet(key, Record.placeholder(Date.now(), Date.now()), lock).catch(console.error);
		...
		for (let i = 0; i < retries; i++) {
			...
			if (cached) {
				...
				if (record.isPlaceholder()) {
					if ((record.placeholder + lock) > Date.now()) {
						await sleep(latency);
					} else {
